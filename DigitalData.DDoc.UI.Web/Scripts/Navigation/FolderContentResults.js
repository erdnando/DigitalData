...
DDocUi.prototype.InitFolderContents = function () {
	...
	ddoc.ResultCollections = resultCollections;

	$.each(ddoc.ResultCollections, function (i, collection) {
		$('.result-grids').append($('<div class="grid-container ' + collection.Id + '"><div id="grd' + collection.Id + '"></div></div>'));
		ddoc['grd' + collection.Id] = new DDoc.Controls.Grid('grd' + collection.Id, { height: 362, showFilter: false, showTools: true, pager: { show: true, pageSize: 10, onServer: false }, thumbnailSrc: ddoc.GetUrl('api/files/getThumbnail/{0}/115/150') });
	...
	ddoc.NavigationTree.filterNodes(function (node) {
		return ddoc.ResultCollections.map(function (col) { return col.Id }).indexOf(node.key) > -1;
	}, { autoExpand: true });
}

var ddoc = new DDocUi();
ddoc.InitFolderContents();